...
# Drop unnecessary columns or features, namely the team name, games played (all are 82), 
# Wins and Losses (win%, same number of games across all seasons)
df_ = df.drop(columns=['TEAM','GP','W','L'])
counter = 0
...
X = df.drop(columns=['CHAMPION','TEAM','GP','W','L','SEASON'])
y = df['CHAMPION']
...
X_2018 = df_2018.drop(columns=['TEAM','GP','W','L'])
